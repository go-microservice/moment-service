...
func (r *postHotRepo) UpdateDelFlag(ctx context.Context, db *gorm.DB, id int64, delFlag int) error {
	err := db.WithContext(ctx).Model(&model.PostHotModel{}).Where("post_id = ?", id).
